<?php 
function generate_google_ad_code() {
...
function insert_ad_in_content($content)
{
    ...
    $ChapterCount = 2; // Enter the paragraph number, which you'd like an ad in front of
    ...
    for ($i = 0; $i < count($content); $i++) {
        if ($i == $ChapterCount) {
            $new_content .= generate_google_ad_code(); // call function for generating ad code
        }
